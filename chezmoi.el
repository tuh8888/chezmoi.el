...
VALUE is ignored."
  (when (and start end)
    (let ((value (get-text-property start 'display object)))
      (remove-text-properties start end `(display ,value) object)
        (font-lock-ensure start end)
        (font-lock-flush start end))))
...
(defun chezmoi--funcall-over-display-properties (f buffer-or-name)
...
the display property value, and BUFFER-OR-NAME."
  (with-current-buffer buffer-or-name
    (let ((start 1)
          (end 1)
          (buf (current-buffer)))
      (while (and (setq start (next-single-property-change end 'display buf))
                  (setq end (next-single-property-change start 'display buf)))
        (funcall f start end buffer-or-name)))))

(defun chezmoi-display-templates (&optional display-p buffer-or-name)
...
Use DISPLAY-P to set display of templates on or off."
  ...
  (let ((buffer-or-name (or buffer-or-name (current-buffer))))
    ...
    (if chezmoi--templates-displayed-p
      ...
      (chezmoi--funcall-over-display-properties #'chezmoi--remove-display-value buffer-or-name))))
...
(defun chezmoi--evil-insert-state-enter ()
  ...
  (let* ((buf (current-buffer))
         (p (point))
         (start (previous-single-property-change p 'display buf))
         (end (next-single-property-change p 'display buf)))
    (chezmoi--remove-display-value start end buf))
  (add-hook 'after-change-functions #'chezmoi--after-change nil 1))
